...
def train_and_evaluate_model(df_for_model):
    ...
    model = Pipeline(
        [
            ...
            ("model", linear_model.Lasso(alpha=0.12)),
        ]
    )
    ...
    feature_importances = model[-1].coef_  # Получаем коэффициенты модели
    ...
    plt.xticks(
        range(X.shape[1]), features[indices], rotation=45, ha="right"
    )  # Угол поворота и выравнивание
